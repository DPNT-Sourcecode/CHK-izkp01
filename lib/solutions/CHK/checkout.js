...
module.exports = function (skus) {
    ...
    if (groupOfferCount >= 3) {
        const reduce = Math.floor(groupOfferCount / 3);
        total += reduce * offers.groupOffer.offPrice;
        groupOfferCount = groupOfferCount % 3;
        let remainingItems = groupOfferCount;
        for (const x in groupItems) {
            if (items[x] > 0) {
    ...
    for (const x of groupItems) {
        if (items[x]) {
            items[x] = Math.max(0, items[x] - Math.floor(items[x] / 3) * 3);
        }
    }
